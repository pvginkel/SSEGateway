...
/**
 * Parse SSE events from response data chunks
 *
 * @param chunks - Array of Buffer chunks received from response
 * @returns Array of parsed SSE events
 */
function parseSseEvents(chunks: Buffer[]): SseEvent[] {
...
describe('SSE Connection Flow', () => {
  ...
  beforeEach(async () => {
    ...
    // Reset response body to default to prevent state leakage between tests
    mockServer.setResponseBody({ status: 'ok' });
  ...
  describe('Connect callback response body', () => {
    it('should send event from callback response', async () => {
      // Configure mock server to return event in callback response
      ...
      // Capture SSE response chunks
      const chunks: Buffer[] = [];
      const req = request(app)
        .get('/sse/test-callback-event')
        .parse((res, callback) => {
      ...
      // Verify event was sent with correct name and data
    ...
    it('should send event with multi-line data from callback response', async () => {
      mockServer.setResponseBody({
        event: {
          name: 'message',
          data: 'Line 1\nLine 2\nLine 3',
      ...
      // Parse SSE events and verify multi-line data
      const events = parseSseEvents(chunks);
      expect(events.length).toBeGreaterThanOrEqual(1);
      expect(events[0]).toEqual({
        name: 'message',
    ...
    it('should send unnamed event from callback response', async () => {
      mockServer.setResponseBody({
        event: {
      ...
      // Parse SSE events and verify unnamed event (no event: line)
      const events = parseSseEvents(chunks);
      expect(events.length).toBeGreaterThanOrEqual(1);
      expect(events[0]).toEqual({
        data: 'Unnamed event data',
    ...
    it('should close connection immediately when callback returns close=true only', async () => {
      mockServer.setResponseBody({
        close: true,
      });

      const req = request(app).get('/sse/test-close-only');
      const responsePromise = req.then(() => {}, () => {});

      // Wait for connection and close
      await new Promise((resolve) => setTimeout(resolve, 300));

      // Verify disconnect callback sent with reason="server_closed"
      const callbacks = mockServer.getCallbacks();
      const disconnectCallback = callbacks.find(
        (cb) => cb.action === 'disconnect' && cb.request.url === '/sse/test-close-only'
      );
    ...
    it('should send event then close when callback returns both', async () => {
      mockServer.setResponseBody({
        event: {
          name: 'goodbye',
      ...
      const req = request(app).get('/sse/test-event-and-close');
      const responsePromise = req.then(() => {}, () => {});
      ...
      const callbacks = mockServer.getCallbacks();
      const disconnectCallback = callbacks.find(
        (cb) => cb.action === 'disconnect' && cb.request.url === '/sse/test-event-and-close'
      );
      expect(disconnectCallback).toBeDefined();
      expect(disconnectCallback!.reason).toBe('server_closed');
    ...
    it('should handle callback response with close=false (no close)', async () => {
      mockServer.setResponseBody({
        event: {
          name: 'test',
          data: 'Test event',
      ...
      const req = request(app).get('/sse/test-close-false');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 500));

      // Verify connection remains open (heartbeats should start)
      const callbacks = mockServer.getCallbacks();
      const connectCallback = callbacks.find(
        (cb) => cb.action === 'connect' && cb.request.url === '/sse/test-close-false'
      );
      expect(connectCallback).toBeDefined();
      expect(connections.has(connectCallback!.token)).toBe(true);
    ...
    it('should handle empty callback response body', async () => {
      mockServer.setResponseBody({});

      const req = request(app).get('/sse/test-empty-body');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Verify connection opened normally
      const callbacks = mockServer.getCallbacks();
      const connectCallback = callbacks.find(
        (cb) => cb.action === 'connect' && cb.request.url === '/sse/test-empty-body'
      );
      expect(connectCallback).toBeDefined();
      expect(connections.has(connectCallback!.token)).toBe(true);
    ...
    it('should handle invalid JSON in callback response body', async () => {
      // Set response body to invalid JSON string (MockServer will JSON.stringify this string)
      mockServer.setResponseBody('invalid json');

      const req = request(app).get('/sse/test-invalid-json');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Verify connection opened normally (invalid body treated as {})
      const callbacks = mockServer.getCallbacks();
      const connectCallback = callbacks.find(
        (cb) => cb.action === 'connect' && cb.request.url === '/sse/test-invalid-json'
      );
      expect(connectCallback).toBeDefined();
      expect(connections.has(connectCallback!.token)).toBe(true);
    ...
    it('should handle callback response with event missing data field', async () => {
      mockServer.setResponseBody({
        event: {
          name: 'invalid',
          // Missing data field
      ...
      const req = request(app).get('/sse/test-missing-data');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Verify connection opened normally (invalid event ignored)
      const callbacks = mockServer.getCallbacks();
      const connectCallback = callbacks.find(
        (cb) => cb.action === 'connect' && cb.request.url === '/sse/test-missing-data'
      );
      expect(connectCallback).toBeDefined();
      expect(connections.has(connectCallback!.token)).toBe(true);
    ...
    it('should handle callback response with close as string "true"', async () => {
      mockServer.setResponseBody({
        close: 'true', // Invalid type - should be boolean
      });

      const req = request(app).get('/sse/test-close-string');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Verify connection opened normally (invalid close ignored)
      const callbacks = mockServer.getCallbacks();
      const connectCallback = callbacks.find(
        (cb) => cb.action === 'connect' && cb.request.url === '/sse/test-close-string'
      );
      expect(connectCallback).toBeDefined();
      expect(connections.has(connectCallback!.token)).toBe(true);
    ...
    it('should handle callback event appearing before first heartbeat', async () => {
      mockServer.setResponseBody({
        event: {
          name: 'first',
          data: 'Callback event',
      ...
      const req = request(app).get('/sse/test-event-before-heartbeat');
      const responsePromise = req.then(() => {}, () => {});
      ...
      const callbacks = mockServer.getCallbacks();
      const connectCallback = callbacks.find(
        (cb) => cb.action === 'connect' && cb.request.url === '/sse/test-event-before-heartbeat'
      );
      expect(connectCallback).toBeDefined();
    ...
    it('should handle client disconnect race condition during callback response processing', async () => {
      mockServer.setResponseBody({
        event: {
          name: 'test',
          data: 'This should not be sent',
        },
        close: true,
      });

      // Use a very short delay to create race condition
      mockServer.setDelay(50);

      const req = request(app).get('/sse/test-race-condition');
      const responsePromise = req.then(() => {}, () => {});

      // Immediately abort to trigger race condition
      ...
      // Verify only one disconnect callback (client_closed, not error)
      const callbacks = mockServer.getCallbacks();
      const disconnectCallbacks = callbacks.filter(
        (cb) => cb.action === 'disconnect' && cb.request.url === '/sse/test-race-condition'
      );

      // Should have at most one disconnect callback
      expect(disconnectCallbacks.length).toBeLessThanOrEqual(1);

      if (disconnectCallbacks.length === 1) {
        // If disconnect happened, it should be client_closed (not error)
        expect(disconnectCallbacks[0].reason).toBe('client_closed');
      }
    }, 10000);
  });

  describe('Disconnect callback response body', () => {
    it('should log warning when disconnect callback returns event', async () => {
      const req = request(app).get('/sse/test-disconnect-event');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Configure mock to return event in disconnect callback
      mockServer.setResponseBody({
        event: {
      ...
      const callbacks = mockServer.getCallbacks();
      const disconnectCallback = callbacks.find(
        (cb) => cb.action === 'disconnect' && cb.request.url === '/sse/test-disconnect-event'
      );
      expect(disconnectCallback).toBeDefined();
      expect(disconnectCallback!.reason).toBe('client_closed');

      // Note: We can't easily verify the warning was logged without capturing logs
      // The important behavior is that the connection closes normally
    }, 10000);

    it('should log warning when disconnect callback returns close=true', async () => {
      const req = request(app).get('/sse/test-disconnect-close');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Configure mock to return close in disconnect callback
      mockServer.setResponseBody({
        close: true,
      });
      ...
      const callbacks = mockServer.getCallbacks();
      const disconnectCallback = callbacks.find(
        (cb) => cb.action === 'disconnect' && cb.request.url === '/sse/test-disconnect-close'
      );
      expect(disconnectCallback).toBeDefined();
    }, 10000);

    it('should handle invalid JSON in disconnect callback response', async () => {
      const req = request(app).get('/sse/test-disconnect-invalid-json');
      const responsePromise = req.then(() => {}, () => {});

      await new Promise((resolve) => setTimeout(resolve, 300));

      // Configure mock to return invalid JSON (will be stringified by MockServer)
      mockServer.setResponseBody('invalid');
      ...
      // Verify disconnect completed (best-effort, no error thrown)
      const callbacks = mockServer.getCallbacks();
      const disconnectCallback = callbacks.find(
        (cb) => cb.action === 'disconnect' && cb.request.url === '/sse/test-disconnect-invalid-json'
      );
      expect(disconnectCallback).toBeDefined();
    }, 10000);
  });
